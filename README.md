...
The installer uses the custom WiX template in `packaging/windows/main.wxs`: if Envelopes is already installed (any version, including the same one), it asks whether to **Replace** it or **Cancel**. Saved addresses in `~/.envelopes` are kept. Compose's built-in `packageMsi` task still works but doesn't include this prompt.
